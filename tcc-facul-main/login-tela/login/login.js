...
confirmarCodigoBtn.addEventListener("click", function() {
    ...
    if (codigoDigitado === window._codigoVerificacaoOwner) {
        ...
        // Substituir a opção de digitar nova senha por um botão de redirecionamento
        const novaSenhaContainer = document.getElementById("novaSenhaContainer");
        novaSenhaContainer.innerHTML = `
            <button id="redirecionarBtn" style="margin-top:10px;">Alterar Senha</button>
        `;
        novaSenhaContainer.style.display = "block";

        // Adicionar evento de redirecionamento ao botão
        document.getElementById("redirecionarBtn").addEventListener("click", function() {
            window.location.href = "/tcc-facul-main/tela-cadastro/editar-perfil/editar-perfil.html"; // Substitua pelo link correto
        });
    } else {
